...
cd $DIR/packaging
rm -rf debian/.debhelper debian/tensorflow-cpp-optimized  debian/tensorflow-cpp-optimized.debhelper.log debian/tensorflow-cpp-optimized.substvars
rm -rf debian/debhelper-build-stamp debian/files
cmake . -DGIT_TAG=${TAG} -DGIT_COMMIT=${COMMIT} -DGIT_DATE=${DATE} -DPKG_SUFFIX=optimized -DCMAKE_INSTALL_PREFIX=/usr
